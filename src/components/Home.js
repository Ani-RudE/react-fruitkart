...
import axios from "axios"; //Axios for HTTP req
import AppContext from "../services/AppContext";

const Home = () => {
     const [state, setState] = useContext(AppContext); //Accessing the Application context
     const [fruits, setFruits] = useState([]);
     const [vegetables, setVegetables] = useState([]);

     const fetchProducts = () => {
          axios.get("http://localhost:5000/freshkart").then((res) => { //Data from local API
               let products = res.data.data;
               for (let product of products) {
                    product.quantity = 0; //Quantity for every fruit
               }
               categorizeProducts(products);
               setState({ ...state, products });
          });

          //Array of fruits info
          let products = [
               {
                    id: 1,
                    name: "Orange",
               ...
               },
               {
                    ...
                    was: 29,
                    weight: "0.5 kg",
                    pic: "http://cdn.shopify.com/s/files/1/0451/1101/7626/products/carrotseds.jpg?",
                    popular: 0,
                    type: "vegetables",
               },
               {
          ...
          for (let product of products) {
               product.quantity = 0; //Quantity property for every fruit
          }
          categorizeProducts(products); //Updates context
          setState({ ...state, products });
     };
     ...
     //fetchProducts() is supposed to be called when the page is loaded, so for that we take useEffect()
     //Might reset state, so we create a context >> AppContext.js
     useEffect(() => {
     ...
     return (
          <>
               <Banner />
               <Cards />
               <Popular title="Popular Fruits" items={fruits} /> {/* Renders popular fruits */}
